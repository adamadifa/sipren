...
<div class="card mt-2">
    <div class="card-body">
        <form action="/karyawan/{{$karyawan->npp}}" method="post">
          ...
          <line x1='19' y1='11' x2='19' y2='13' /></svg>" value="{{$karyawan->npp}}" />
                </div>
            </div>
            <div class="row mb-2">
                <div class="col-md-6">
            ...
            <div class="row mb-2">
                ...
                <div class="col-md-4">
                   <div class="form-group @error('status') is-invalid @enderror">
                        <select name="golongan_darah" id="golongan_darah" class="form-select @error('status') is-invalid @enderror">
                            <option value="">Golongan Darah</option>
                            <option @isset($karyawan->golongan_darah) @if(old("golongan_darah")) {{old("golongan_darah")=="A" ? "selected":""}}
                                @else
                                {{$karyawan->golongan_darah=="A" ? "selected":""}} @endif @else
                                {{old("golongan_darah")=="A" ? "selected":""}}
                                @endisset value="A">A</option>
                            <option @isset($karyawan->golongan_darah) @if(old("golongan_darah")) {{old("golongan_darah")=="B" ? "selected":""}}
                                @else
                                {{$karyawan->golongan_darah=="B" ? "selected":""}} @endif @else
                                {{old("golongan_darah")=="B" ? "selected":""}}
                                @endisset value="B">B</option>
                            <option @isset($karyawan->golongan_darah) @if(old("golongan_darah")) {{old("golongan_darah")=="AB" ? "selected":""}}
                            @else
                            {{$karyawan->golongan_darah=="AB" ? "selected":""}} @endif @else
                            {{old("golongan_darah")=="AB" ? "selected":""}}
                            @endisset value="AB">AB</option>
                            <option @isset($karyawan->golongan_darah) @if(old("golongan_darah")) {{old("golongan_darah")=="O" ? "selected":""}}
                            @else
                            {{$karyawan->golongan_darah=="O" ? "selected":""}} @endif @else
                            {{old("golongan_darah")=="O" ? "selected":""}}
                            @endisset value="O">O</option>
                ...
                <div class="col-md-12">
                    <div class="form-group @error('status') is-invalid @enderror">
                        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                            ...
                            <option @isset($karyawan->status) @if(old("status")) {{old("status")=="M" ? "selected":""}}
                                @else
                                {{$karyawan->status=="M" ? "selected":""}} @endif @else
                                {{old("status")=="M" ? "selected":""}}
                                @endisset value="M">Menikah</option>
                            <option @isset($karyawan->status) @if(old("status")) {{old("status")=="BM" ? "selected":""}}
                                @else
                                {{$karyawan->status=="BM" ? "selected":""}} @endif @else
                                {{old("status")=="BM" ? "selected":""}}
                                @endisset value="BM">Belum Menikah</option>
                                <option @isset($karyawan->status) @if(old("status")) {{old("status")=="PM" ? "selected":""}}
                                @else
                                {{$karyawan->status=="PM" ? "selected":""}} @endif @else
                                {{old("status")=="PM" ? "selected":""}}
                                @endisset value="PM">Pernah Menikah</option>
                        </select>
                    </div>
                    @error('status') <div class="mb-2 mt-2 invalid-feedback">{{$message}}</div> @enderror
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <div class="form-group  @error('id_propinsi') is-invalid @enderror">
                        <select name="id_propinsi" id="id_propinsi"
                            ...
                            <option @isset($karyawan->id_propinsi) @if(old("id_propinsi"))
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group @error('status_kepegawaian') is-invalid @enderror">
                        <select name="status_kepegawaian" id="status_kepegawaian"
                            ...
                            <option @isset($karyawan->status_kepegawaian) @if(old("status_kepegawaian"))
                            ...
                            <option @isset($karyawan->status_kepegawaian) @if(old("status_kepegawaian"))
                                ...
                                @endisset value="Kontrak">Kontrak</option>
                            <option @isset($karyawan->status_kepegawaian) @if(old("status_kepegawaian"))
                                ...
                                @endisset value="Tetap">Pegawai Tetap</option>
                            <option @isset($karyawan->status_kepegawaian) @if(old("status_kepegawaian"))
                                ...
                                @endisset value="Pengabdian">Pengabdian</option>
                        </select>
                    </div>
                    @error('status_kepegawaian') <div class="mb-2 mt-2 invalid-feedback">{{$message}}</div> @enderror
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group @error('unit') is-invalid @enderror">
                        <select name="id_unit" id="id_unit" class="form-select @error('id_unit') is-invalid @enderror">
                            ...
                            @foreach ($unit as $u)
                            <option @isset($karyawan->id_unit) @if(old("id_unit"))
                                {{old("id_unit")==$u->id ? "selected":""}} @else
                                {{$karyawan->id_unit== $u->id ? "selected":""}} @endif @else
                                {{old("id_unit")==$u->id ? "selected":""}}
                                @endisset {{old('id_unit')==$u->id ? 'selected':''}}
                                value="{{$u->id}}">{{$u->nama_unit}}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="row mb-2">
                <div class="col-md-6">
                    <x-inputtext label="Nama Ayah" placeholder="Nama Ayah" field="nama_ayah"
                        value="{{$karyawan->nama_ayah}}" icon='<svg xmlns=" http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24"
            ...
            <div class="rom mb-2">
                <x-inputtextarea label="Alamat Orang tua" field="alamat_orangtua"
                    value="{!! $karyawan->alamat_orangtua !!}" />
            </div>
            <div class="row mb-2">
                <div class="col-md-12">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group  @error('pendidikan_terakhir') is-invalid @enderror">
                        <select name="pendidikan_terakhir" id="pendidikan_terakhir"
                            ...
                            <option @isset($karyawan->pendidikan_terakhir)
                    ...
                    @error('pendidikan_terakhir') <div class="mb-2 mt-2 invalid-feedback">{{$message}}</div> @enderror
                </div>
                <div class="col-md-6">
                    <x-inputtext label="Pekerjaan" placeholder="Pekerjaan" field="pekerjaan"
                        value="{{$karyawan->pekerjaan}}" icon='<svg xmlns="
            ...
            <div class="row mb-2">
                <div class="col-md-12">
                    <x-inputtext label="Kontak" placeholder="Kontak" field="kontak" value="{{$karyawan->kontak}}" icon='<svg xmlns=" http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24"
            stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
            <path
